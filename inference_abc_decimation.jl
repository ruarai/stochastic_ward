...
function run_inference(
    ...
    for i in 1:num_thresholds
        ...
        Threads.@threads for p in 1:num_candidates
            ...
            candidate_errors[i, p] = get_error(best_candidate, true_occupancy_matrix, occupancy_weighting, n_days, n_steps_per_day)

            for j in 1:(num_stochastic_samples - 1)
                test_candidate = model_process(1, candidate_sigmas[p], context, Random.MersenneTwister())
                test_candidate_error = get_error(test_candidate, true_occupancy_matrix, occupancy_weighting, n_days, n_steps_per_day)
...
function get_error(state, true_occupancy, occupancy_weighting, n_days, n_steps_per_day)
    error = 0

    for d in 1:n_days
        t = (d - 1) * n_steps_per_day + 1
        sim_occupancy = get_total_ward_occupancy(state, t, d)

        if true_occupancy[d, 1] > -0.5
            error += abs.(sim_occupancy - true_occupancy[d, 1]) * occupancy_weighting[d]
        end
